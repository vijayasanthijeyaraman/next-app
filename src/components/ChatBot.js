import { useChatBot } from "EAMBlog/core/hooks";
...
export const ChatBot = ({ isFullScreen = true }) => {
  ...
  return (
    <div className="w-full h-full max-w-screen-xl mx-auto">
      <div className="w-full h-full p-2 space-y-4 flex flex-col justify-end rounded-lg bg-white border border-purple-500 ">
        <div className="w-full h-full p-3 overflow-y-auto rounded-lg border-2 border-purple-400 space-y-5">
          {conversations.slice(1)?.map(({ role, content }, index) => {
            return (
              <div key={index} className={`flex ${role === "user" ? "justify-end" : ""}`}>
                {role === "user" ? (
                  <span className="flex gap-2">
                    <span className="max-w-lg px-4 py-2 font-medium text-white rounded-lg bg-purple-600">
                      {content}
                    </span>
        ...
        <div className="w-full px-3 sm:px-4 py-1.5 flex gap-3 items-end rounded-lg bg-white border-2 border-purple-400">
          <textarea
